...
def question_4(all_soup):
    ...
    for soup in all_soup:
        ...
        if artist in object_dicts:
            object_dicts[artist]['works'].append({'title' : work, 'currenncy' : price[0] , 
                                                  'amount' : price[1]})
        else:
...
def dump_json(obj_list):
    ...
    with open('problem4_2.json', 'w') as outfile:
...
# caller
dump_json(question_4(read_soup(file_names)))
